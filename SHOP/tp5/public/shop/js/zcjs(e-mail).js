...
window.onload = function(){
	...
	/*----------手机号码验证----------*/
		...
		function emm(e1){  //e1事件对象
		var type;  //文本框事件类型
		if(e1){
			type =e1.type;
		}
		var v = email.value; //文本框的值
		var tp = email.parentNode; //文本框容器节点对象tp
		var tip = tp.nextElementSibling;//提示信息容器节点对象
		var span = tip.lastElementChild; //存放提示信息文本的节点对象
		if(type=="focus"){
		...
		if(type=="blur"){
			if(v.length==0){
				tp.className = "tp";
					emd0.className = "emd0 cuo"
					tip.className = "tip error";
					email.className = "email act";
					span.innerHTML = "请输入邮箱"
				return false;
			}else if(!regs.remail.test(v)){
	...
	btn.onclick = function(){
			...
			if(	
				...
				if(confirm("注册成功！")){
					...
					localStorage.setItem(Email,emailnum);
